    ...
    <custom-container :croute="croute" :hide="false">
      <b-table :data="pageList.items">
        <template slot-scope="props">
          <b-table-column label="Date" width="10">
            <nuxt-link
              :to="'/blog/' + props.row.slug"
            >{{ props.row.created_at | moment('DD/MM/YYYY') }}</nuxt-link>
          </b-table-column>

          <b-table-column label="Title">
            <nuxt-link :to="'/blog/' + props.row.slug">{{props.row.title}}</nuxt-link>
          </b-table-column>

          <b-table-column label="Author" numeric>
          ...
          <b-table-column label=" " class="has-text-right">
            <nuxt-link :to="'/blog/' + props.row.id" class="mdi mdi-eye icobtn" />
